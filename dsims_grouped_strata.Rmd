---
title: "Distance sampling simulations with grouped strata"
description: |
  Description and R code to produce simulations to demonstrate the case when you have multiple strata in the design which you wish to be grouped for the analysis. This situation arises when you need to stratify purely for design purposes. This example contains more advanced simulation options, it is assumed that readers have some familiarity with `dssd` and `dsims` already, our other vignettes provide a more introductory level to design and simulation.
author:
  - name: Laura Marshall 
...
Currently `dssd` [@dssdpkg] and `dsims` [@dsimspkg] must be installed from our github repositories rather than CRAN. The code below first installs `devtools` [@devtoolspkg] from CRAN and then `dssd` and `dsims` from github.

```{r packages_temp, echo=FALSE, eval=FALSE}
# Package not currently on CRAN (alternative install below)
...
In addition to the R packages, there are additional files required by this simulation. All necessary material are included in a Zip archive file entitled [results.zip](results.zip). Uncompressing the contents into a folder called results  within the same folder as the .Rmd file should give you the required structure to run the code in the .Rmd file. You should end up with the file *sim.results.ROBJ* within the results folder. 
...
Please note, that while this study area has already been divided into numerous strata, this step would normally be carried out later at the design stage to allow optimisation of the design. For this example, let's imagine that in reality we are interested in estimates for only two distinct areas in this study region, these will consist of the three northern strata grouped together and the three southern strata grouped together. 
...
As mentioned above, we have two sub regions of interest in this study area, for which we would like estimates of density / abundance (the northern three strata and the southern three strata). Let's start by constructing our design as though we had only divided our study region into two strata. We expect more animals in the southern strata so we will allocate more effort per unit area to this strata, i.e. higher coverage. 

Let's assume that our effort calculations have suggested that we have sufficient resources to survey parallel lines with a spacing of 16,000m in the northern strata and a spacing of 8,000m in the southern strata. We then need to supply a single design angle for the northern strata and one for the southern strata, let's set these to be 135 and 70 degrees, respectively. We will also specify that we will be doing minus sampling and do not expect to observe animals beyond 1,500m.

We will generate a set of transects from this design and assess them for desirable design qualities.

```{r design_one, echo=myecho, eval=myeval}
# Define a design based on only two strata
design <- make.design(region = region,
                      transect.type = "line",
                      design = "systematic",
                      spacing = c(rep(16000,3),rep(8000,3)),
                      design.angle =  c(rep(135,3),rep(70,3)),
                      edge.protocol = "minus",
                      truncation = 1500)
...
We can see from this first design, given the complexity of the region, choosing a single design angle for the northern and southern groups of strata is not going to achieve this goal. This is particularly problematic in the southern strata where selecting a design angle to give lines perpendicular to the coast in one area gives lines that are parallel to the coast in another. It is at this point, that it becomes clear that dividing the region further will better optimise the design. We will now specify individual design angles for each of the six strata.

```{r design_two, echo=myecho, eval=myeval}
# Define the design
design <- make.design(region = region,
                      transect.type = "line",
                      design = "systematic",
...
```
We can now create a density grid with a spacing of 2,500m in both dimensions and add two hotspots to simulate a potentially realistic distribution of animals which prefer to stick closely to the coast. 

```{r density, echo=myecho, eval=myeval}
...
We will define a basic population for this simulation resulting in 2500 individuals. As the make.population command requires us to specify how many individuals per stratum, we will have to calculate this using the density summary.

```{r density_summary, echo=myecho, eval=myeval}
# View the density summary
summary(my.density)
```
We can see from the table that if we used the exact densities in the density grid we would generate a lot of animals (see ave.N column)! What we often do, as it is fairly difficult to define a density surface with exact density values, is to use this as a relative density surface. We can therefore decide how many animals to allocate to each stratum by scaling these values.  
...
group.strata
```
We will now define the analyses. It is advisable to incorporate some model uncertainty into the simulations so we will allow the simulation to select between a half normal and a hazard rate model both with strata group included as a covariate. We will use the AIC as the criteria for model selection.
...
We now have all the components we need to group them together as a simulation. At this point we also select the number of repetitions which for this example we will set to 1000. Note that the first time you run a simulation you should limit the number of repetitions to only a few to check everything works as expected.
...
```{r egsurvey, echo=myecho, eval=myeval, warning=FALSE}
...
Next we will view the table giving the abundance estimates. There is a small amount of negative bias for both strata and in the total estimate of abundance. The bias does seem to be sufficient to reduce the coverage of the confidence intervals (which should be 0.95) to only 0.92 for the southern strata and the total estimate. Sometimes reduced confidence interval coverage can be due to the variance being under estimated but in this case it seems only related to the bias for the southern strata and only slightly amplified in the total. The mean.se (mean of the estimated standard error) and the sd.of.means (truth - observed standard error of the estimates) are very close for the southern strata and only indicate very slight under estimation for the total.
...
Even though the systematic parallel line design should give us largely uniform coverage across the region, because we are using minus sampling there is still the chance that edge effects could affect the estimates. This regions does have a long perimeter in comparison to the area contained within it. A coverage simulation will help to give an indication of how much reduced coverage there is around the boundary but to more accurately check whether the bias in the results and the reduced confidence interval coverage could be associate with edge effects we could repeat the survey with a uniform density surface. Edge effects will not cause bias when the density of animals around the edge of the region is the same as in the middle of the region. If the bias is due to edge effects then remember that the above case is likely not a worst case scenario and so it may be worth testing other plausible animal distributions to see if more severe bias is possible.
